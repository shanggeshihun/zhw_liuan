...
def calc_tb_hb(type='ratio',numeric=0,compare_numeric=0):
    ...
    :return:同环比结果
    '''
    numeric, compare_numeric = float(numeric), float(compare_numeric)
    if type == 'value':
        if float(compare_numeric) == 0:
            if float(numeric)>0:
                return 1
            else:
                return 0
        else:
            return float(numeric)/compare_numeric - 1
    else:
...
def float_to_percent_nosign(float_numeric):
    float_numeric = float(float_numeric)
...
if warning_modules == 0 :
    ...
    work_wxrobot(content, prod_webhook)