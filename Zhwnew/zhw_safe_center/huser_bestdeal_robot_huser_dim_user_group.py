...
# @Time      : 2023/7/6  15:34
# @Author    : An
# @File      : huser_bestdeal_robot_huser_dim_user_group.py
...
def huser_bestdeal_robot_huser_dim_user_group(start_day, end_day, group_id):
    '''
    :param start_day:
    :param end_day:
    :param group_id:
    ...
    holo_sql = """    	
        ...
        select 
            ...
            sum(a.pm) as pm,
            coalesce(count(case when a.zt = 3 then a.id end),0) as cd_orders,
            coalesce(sum(case when a.zt = 3 then a.pm end),0) as cd_pm,
            coalesce(count(b.order_id),0) as hb_orders,
            coalesce(sum(b.use_money),0) as hb_use_money
...
if __name__ == '__main__':
    ...
    """2023-07-13，
    group_name 号主刷红包-ROBOT商户-用户逻辑 58
    """
    group_id = 58
    group_id = 999
    ...
    df_final_report = huser_bestdeal_robot_huser_dim_user_group(start_day, end_day, group_id)
