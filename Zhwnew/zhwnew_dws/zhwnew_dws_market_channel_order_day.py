...
class ZhwnewDwsMarketChanelOrderDay:
    def __init__(self):
    ...
    def run(self, start_day ,end_day):
        ...
        # 清理目标表数据
        mysql_sql = "delete from zhwnew_dws_market_channel_order_day where part_day between '{0}' and '{1}'".format(start_day ,end_day)
        ...
        holo_sql = """
            ...
            with market_user_tmp as (
                    ...
                    select jkx_userid,max(jkx_day) as jkx_day,
                    max(big_channel) as big_channel,max(channel_type) as channel_type,
                    max(channel_id) as channel_id,max(channel) as channel
                    from 
                    (
                        select a.jkx_userid,to_char(a.jkx_timer,'yyyy-mm-dd') as jkx_day,
                        b.big_channel,b.channel_type,cast(b.browser as text) as channel_id,b.browser as channel
                        from ods_zhw.zhw_search_record a
                        join 
                        (
                            select 
                                'SEM端' as big_channel,
                                case when browser in ('品牌专区','360品专','百度专区') then '品牌专区' else 'SEM搜索' end as  channel_type,
                                id as channel_id,search_word as channel,browser
                            from ods_zhw.zhw_search_word 
                            where true 
                            and (browser ~'品牌专区|360品专|百度专区|搜索')
                        ) b
                        on a.wordid = b.channel_id
                        ...
                        select a.jkx_userid, a.jkx_timer as jkx_day,
                        d.big_channel,d.channel_type,d.channel_id,d.channel as browser
                        from ods_zhw.zhw_bigdata_market_user a
                        left join 
                        (
                            select 'SEM端' as big_channel,
                            'SEM搜索' as  channel_type,
                            app_channel as channel_id,app_channel_name as channel 
                            from ods_zhw.zhw_bi_channel_config
                            where status=1 
                            and lower(name) in ('sem搜索') 
                        ) d 
                    ...
                    left join 
                    (
                        select '移动端' as  big_channel,lower(name) as channel_type,app_channel as channel_id,app_channel_name as channel 
                        from ods_zhw.zhw_bi_channel_config
                        where status=1 
                        and lower(name) in ('app安卓','apppro','applite','appulite','applite') 
                        group by 1,2,3,4
                    ) e 
            ...
            gmv_tmp as (
                ...
                (
                    ...
                    and part_day between '{0}' and to_char(date('{0}') + interval '30 days','yyyy-mm-dd')
                ) b 
                ...
                (
                    ...
                    and to_char(add_time ,'yyyy-mm-dd') between '{0}' and to_char(date('{0}') + interval '30 days','yyyy-mm-dd')
                ) c
            ...
            bt_tmp as (
                ...
                (
                    ...
                    and to_char(create_time,'yyyy-mm-dd') between '{0}' and to_char(date('{0}') + interval '30 days','yyyy-mm-dd')
                    group by 1,2
                ) x 
            ...
            select 
                coalesce(a.part_day,b.part_day) as part_day,
            ...
            from gmv_tmp a 
            full join bt_tmp b 
            on a.part_day = b.part_day and a.big_channel = b.big_channel
        ...
        # 目标表 数据写入
        columns = 17
        insert_sql = "insert into zhwnew_dws_market_channel_order_day(part_day ,big_channel ,channel_type ,channel ,channel_id ,reg_orders ,old_orders ,order_money ,s_fee ,c_fee ,add_fee ,sub_add_fee ,reg_fee ,old_fee ,cd_orders ,use_hb_money ,sys_spend) values ({0});".format(','.join(['%s'] * columns))
...
if __name__ == '__main__':
    ...
    start_day, last_day = '2023-02-28','2023-02-28'
    zhwnew_dws_market_channel_order_day = ZhwnewDwsMarketChanelOrderDay()
    zhwnew_dws_market_channel_order_day.run(start_day ,last_day)
    end_time = time.time()
    ...
    #     zhwnew_dws_market_channel_order_day = ZhwnewDwsMarketChanelOrderDay()
    #     zhwnew_dws_market_channel_order_day.run(last_day ,last_day)
    #     end_time = time.time()
    #     print(last_day, '运行耗时：', end_time - start_time)
    #     time.sleep(3)