...
if __name__ == '__main__':
    ...
    sql_file_name = 'close_user_dedit_money'
    df = pd.DataFrame()

    start, end = '2023-06-12', '2023-06-15'
    date_range = pd.date_range(start=start, end=end)
    a = [d.strftime('%Y-%m-%d') for d in date_range.tolist()][:-2]
    b = [d.strftime('%Y-%m-%d') for d in date_range.tolist()][2:]
