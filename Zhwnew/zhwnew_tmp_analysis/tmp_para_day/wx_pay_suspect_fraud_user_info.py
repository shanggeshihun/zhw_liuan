...
if __name__ == '__main__':
    df = pd.DataFrame()
    sql_file_name = 'wx_pay_suspect_fraud_user_info'
    ...
    for i in range(20):
        tmp_day = (datetime.datetime.now() - datetime.timedelta(days=i)).strftime('%Y-%m-%d')  # h

        start_time = time.time()

        start_day, end_day = tmp_day, tmp_day
        ...
        print(tmp_day, sql_file_name, '，运行耗时：%s s' % (end_time - start_time))

    df.to_excel(r"./tmp_day_minute_data/{0}.xlsx".format(sql_file_name))