...
-- 	cast (now() as timestamp) as push_time
	...
	t1.m_stimer as "订单开始时间",t1.m_etimer as "订单结束时间",t1.m_userid as "用户名",t1.addfrom_name as "订单来源"
from
(
...
(	-- 昨日之前 推送的数据
	...
	and substring(push_time,1,10)<to_char(date('{0}') - interval '1 days','yyyy-mm-dd')
) t4
