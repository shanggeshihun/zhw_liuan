...
if __name__ == '__main__':
    ...
    while True:
        today = (datetime.datetime.now()).strftime('%Y-%m-%d')  # 今日日期
        last_day = (datetime.datetime.now() - datetime.timedelta(days=1)).strftime('%Y-%m-%d')  # h
        now_hour = (datetime.datetime.now()).strftime('%H:%M:%S')

        if now_hour == '00:00:00' or now_hour == '12:00:00' or now_hour == '19:29:00':

            sql_file_name = 'target_44_push'
            df = pd.DataFrame()

            start_end_day_list = [(last_day,today)]
            ...
            # 您好！
            # 附件推送时间：{}
            # 策略44_手游封号_订单匹配推送 见附件：
            # """
            # content = content_template.format(current_time)
            #
            # qqexmail_smtp = QqExmailSmtp('anliu@dofun.cn', 'Shui1520')
            # qqexmail_smtp.connect()
            #
            # to_addrs = ['anliu@dofun.cn','paullv1@dofun.cn','belleyang@dofun.cn']
            # to_addrs = ['anliu@dofun.cn']
            #
            # subject = '策略44_手游封号_订单匹配推送'
            #
            # attachment_path = r"E:\工作文件\在刀锋\dofun\Python\Python\zhw_liuan\Zhwnew\zhwnew_tmp_analysis\tmp_para_day\tmp_day_data\{0}_{1}.xlsx".format(sql_file_name,current_time)
