...
# @Author    : An
# @File      : strategy_withdraw_8_90pp_r4.py
# @Software  : 近3日订单量>=8单且非上号器原因导致撤单率>0.9
...
if __name__ == '__main__':
    ...
    sql_file_name = 'strategy_withdraw_8_90pp_r4'
    df = pd.DataFrame()

    start, end = '2023-04-01', '2023-06-19'
    date_range = pd.date_range(start=start, end=end)
    a = [d.strftime('%Y-%m-%d') for d in date_range.tolist()][:-3]
    b = [d.strftime('%Y-%m-%d') for d in date_range.tolist()][3:]

    start_end_day_list = list(zip(a,b))
    ...
    df.to_excel(r"./tmp_day_data/{0}_{1}_{2}.xlsx".format(sql_file_name,start, end))