...
if __name__ == '__main__':
    ...
    sql_file_name = 'strategy_withdraw_8_90pp_hour'
    df = pd.DataFrame()

    date_hour_range = pd.date_range(start='2023-05-19', end='2023-05-23', freq='h')
    a = [d.strftime('%Y-%m-%d') for d in date_hour_range.tolist()][:-2]
    b = [d.strftime('%Y-%m-%d') for d in date_hour_range.tolist()][2:]
    c = [d.strftime('%Y-%m-%d %H:00:00') for d in date_hour_range.tolist()][:-2]
    d = [d.strftime('%Y-%m-%d %H:00:00') for d in date_hour_range.tolist()][2:]
    ...
    df.to_excel(r"./tmp_day_minute_data/{0}_{1}.xlsx".format(sql_file_name,current_time))