...
# @Author    : An
# @File      : wow_dwm_recharge_refund_measures_by_refundday.py
# @Software  : PyCharm
...
class WowDwmRechargeRefundMeasuresByRefundday:
    def __init__(self):
    ...
    def run(self, start_day, end_day):
        ...
        # 清理目标表数据
        mysql_sql = "delete from wow_dwm_recharge_refund_measures_by_refundday where part_day between '{0}' and '{1}'".format(start_day, end_day)
        ...
        # 目标表 数据写入
        columns = 12
        insert_sql = "insert into wow_dwm_recharge_refund_measures_by_refundday(part_day ,pay_method ,source_name ,recharge_status ,big_business ,business ,recharge_money ,recharge_times ,recharge_users ,refund_money ,refund_times ,refund_users) values ({0});".format(','.join(['%s'] * columns))
...
if __name__ == '__main__':
    ...
    for i in lst:
        ...
        wow_dwm_recharge_refund_measures_by_refundday.run(last_day, last_day)
        end_time = time.time()
