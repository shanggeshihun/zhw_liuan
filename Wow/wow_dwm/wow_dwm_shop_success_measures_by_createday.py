...
# @Author    : An
# @File      : wow_dwm_shop_success_measures_by_createday.py
# @Software  : wow_dwm_shop_success_measures_by_createday 不涉及 滞后更新的数据指标
...
class WowDwmShopSuccessMeasuresByCreateday:
    def __init__(self):
    ...
    def run(self, start_day, end_day):
        ...
        # 清理目标表数据
        mysql_sql = "delete from wow_dwm_shop_success_measures_by_createday where part_day between '{0}' and '{1}'".format(start_day, end_day)
        ...
        # 目标表 数据写入
        columns = 12
        insert_sql = "insert into wow_dwm_shop_success_measures_by_createday(part_day ,pay_method ,source_name ,sxtype_name ,pay_money ,pay_wabao ,pay_give_wabao ,orders ,users ,pay_money_users ,pay_wabao_users ,pay_give_wabao_users) values ({0});".format(
            ','.join(['%s'] * columns))
...
if __name__ == '__main__':
    ...
    for i in lst:
        ...
        wow_dwm_shop_success_measures_by_createday.run(last_day, last_day)
        end_time = time.time()
