...
# @Author    : An
# @File      : wow_dwm_shop_and_war_measures_by_payday.py
# @Software  : wow_dwm_shop_and_war_measures_by_payday 不涉及 滞后更新的数据指标
...
class WowDwmShopAndWarMeasuresByPayday:
    def __init__(self):
    ...
    def run(self, start_day, end_day):
        ...
        # 清理目标表数据
        mysql_sql = "delete from wow_dwm_shop_and_war_measures_by_payday where part_day between '{0}' and '{1}'".format(start_day, end_day)
        ...
        (
            select 
                ...
                count(distinct uid) as users,count(1) as orders
            from 
            (
                select uid,part_day,coalesce(p.pay_way,99999999) as pay_type,a.app_id
                from 
                (
                    select uid,to_char(addtime,'yyyy-mm-dd') as part_day,recharge_id,
                ...
                union all
                select uid,to_char(success_time,'yyyy-mm-dd') as part_day,pay_type,
                case when app_id is null or app_id = '' then '99999999' else app_id end as app_id
            ...
            group by grouping sets((1),(1,2),(1,3))
        ) t 
        ...
        # 目标表 数据写入
        columns = 5
        insert_sql = "insert into wow_dwm_shop_and_war_measures_by_payday(part_day ,pay_method ,source_name ,users ,orders) values ({0});".format(
            ','.join(['%s'] * columns))
...
if __name__ == '__main__':
    ...
    for i in lst:
        ...
        wow_dwm_shop_and_war_measures_by_payday.run(last_day, last_day)
        end_time = time.time()
