...
# @Author    : An
# @File      : wow_dwm_shop_measures_by_refundday.py
# @Software  : wow_dwm_shop_measures_by_refundday 不涉及 滞后更新的数据指标
...
class WowDwmShopMeasuresByRefundday:
    def __init__(self):
    ...
    def run(self, start_day, end_day):
        ...
        # 清理目标表数据
        mysql_sql = "delete from wow_dwm_shop_measures_by_refundday where part_day between '{0}' and '{1}'".format(start_day, end_day)
        ...
        rf_pay_money ,rf_pay_wabao ,rf_pay_give_wabao ,rf_orders ,rf_users ,rf_cost ,rf_buy_num
        from 
        (
            select 
                ...
                sum(a.pm_wabao) as rf_pay_wabao,
                sum(a.pm_give_wabao) as rf_pay_give_wabao,
                count(a.uid) as rf_orders,
                count(distinct a.uid) as rf_users,
                sum(a.pm_buy*a.buy_num/100.00) as rf_cost,
                sum(a.buy_num) as rf_buy_num
            from 
            (
                ...
                where true 
                and to_char(update_time,'yyyy-mm-dd') between '{0}' and '{1}'
                and status = 6 
            ) a 
            left join 
            (	-- 支付方式
        ...
        # 目标表 数据写入
        columns = 11
        insert_sql = "insert into wow_dwm_shop_measures_by_refundday(part_day ,pay_method ,source_name ,sxtype_name ,rf_pay_money ,rf_pay_wabao ,rf_pay_give_wabao ,rf_orders ,rf_users ,rf_cost ,rf_buy_num) values ({0});".format(
            ','.join(['%s'] * columns))
...
if __name__ == '__main__':
    ...
    for i in lst:
        ...
        wow_dwm_shop_measures_by_refundday.run(last_day, last_day)
        end_time = time.time()
