...
# @Author    : An
# @File      : wow_dwm_shop_measures_by_sendday.py
# @Software  : wow_dwm_shop_measures_by_sendday 不涉及 滞后更新的数据指标 已核对
...
class WowDwmShopMeasuresBySendday:
    def __init__(self):
    ...
    def run(self, start_day, end_day):
        ...
        # 清理目标表数据
        mysql_sql = "delete from wow_dwm_shop_measures_by_sendday where part_day between '{0}' and '{1}'".format(start_day, end_day)
        ...
        send_fee,send_cost,send_orders,send_users,send_nums
        from 
        (
            select 
                ...
                sum(a.send_pm/100.00) as send_fee,
                sum(a.pm_buy*buy_num/100.00) as send_cost,
                count(a.uid) as send_orders,
                count(distinct a.uid) as send_users,
                sum(a.buy_num) as send_nums
            from 
            (
                select shop_type,pm,pm_wabao,pm_give_wabao,uid,send_pm,pm_buy,buy_num,pay_type,
                case when app_id is null or app_id = '' then '99999999' else app_id end as app_id,
                to_char(send_time,'yyyy-mm-dd') as part_day
        ...
        # 目标表 数据写入
        columns = 17
        insert_sql = "insert into wow_dwm_shop_measures_by_sendday(part_day ,pay_method ,source_name ,sxtype_name ,pay_money ,pay_wabao ,pay_give_wabao ,orders ,users ,pay_money_users ,pay_wabao_users ,pay_give_wabao_users,send_fee,send_cost,send_orders,send_users,send_nums) values ({0});".format(
            ','.join(['%s'] * columns))
...
if __name__ == '__main__':
    ...
    for i in lst:
        ...
        wow_dwm_shop_measures_by_sendday.run(last_day, last_day)
        end_time = time.time()
