...
# @Author    : An
# @File      : wow_dwm_shop_and_war_users.py
# @Software  : wow_dwm_shop_and_war_users 不涉及 滞后更新的数据指标
...
class WowDwmShopAndWarUsers:
    ...
    def run(self, start_day, end_day):
        ...
        # 清理目标表数据
        mysql_sql = "delete from wow_dwm_shop_and_war_users where part_day between '{0}' and '{1}'".format(start_day, end_day)
        operate_mysql.delete_data(mysql_sql)

        # 原始表 数据处理
        holo_sql = """
            select tt.part_day,coalesce(tt.source_name,'-') as source_name,tt.users
            from 
            (
                select t.part_day,coalesce(d2.item_name,'其他') as source_name,
                ...
                on cast(t.app_id as integer)  = d2.item_value 
        ...
        # 目标表 数据写入
        columns = 3
        insert_sql = "insert into wow_dwm_shop_and_war_users(part_day ,source_name ,users) values ({0});".format(','.join(['%s'] * columns))
...
if __name__ == '__main__':
    ...
    for i in lst:
        ...
        wow_dwm_shop_and_war_users.run(last_day, last_day)
        end_time = time.time()
