...
# @Author    : An
# @File      : wow_dwm_main_behavior_by_eventday.py
# @Software  : wow_dwm_main_behavior_by_eventday 不涉及 滞后更新的数据指标
...
class WowDwmMainBehaviorByEventday:
    def __init__(self):
    ...
    def run(self, start_day, end_day):
        ...
        # 清理目标表数据
        mysql_sql = "delete from wow_dwm_main_behavior_by_eventday where part_day between '{0}' and '{1}'".format(start_day, end_day)
        operate_mysql.delete_data(mysql_sql)

        # 原始表 数据处理
        holo_sql = """
        select part_day,event_name,coalesce(source_name,'-') as source_name,users,devices
        from 
        (
            ...
            count(null) as devices
            from 
            (   -- 点击登录
                select addtime,uid,case when app_id is null or app_id = '' then '99999999' else app_id end as app_id,
                'login' as event_name
            ...
            a.event_name,
            coalesce(d2.item_name,'其他') as source_name,
            count(distinct a.device_id) as users,
            count(null) as devices
            from 
            (
                select addtime,uid,device_id,
        ...
        # 目标表 数据写入
        columns = 5
        insert_sql = "insert into wow_dwm_main_behavior_by_eventday(part_day ,event_name ,source_name ,users ,devices) values ({0});".format(','.join(['%s'] * columns))
...
if __name__ == '__main__':
    ...
    for i in lst:
        ...
        wow_dwm_main_behavior_by_eventday.run(last_day, last_day)
        end_time = time.time()
