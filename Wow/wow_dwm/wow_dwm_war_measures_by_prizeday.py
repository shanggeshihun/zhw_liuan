...
# @Author    : An
# @File      : wow_dwm_war_measures_by_prizeday.py
# @Software  : wow_dwm_war_measures_by_prizeday 涉及 滞后更新的数据指标 手动回收，自动回收 已核对
...
class WowDwmWarMeasuresByPrizeday:
    def __init__(self):
    ...
    def run(self, start_day, end_day):
        ...
        # 清理目标表数据
        mysql_sql = "delete from wow_dwm_war_measures_by_prizeday where part_day between '{0}' and '{1}'".format(start_day, end_day)
        ...
        recovery_num ,recovery_times ,recovery_users ,auto_recovery_num ,auto_recovery_times ,auto_recovery_users ,manual_recovery_num ,manual_recovery_times ,manual_recovery_users
        from 
        (
            select 
                ...
                case when a.is_free = 1 then '是' else '否' end as is_free,
                
                sum(a.pm/100.00) as prize_money,
                count(a.id) as prize_times,
                count(distinct a.uid) as prize_users,
                
                sum(a.pm_red_packet/100.00) as use_red_packet_money,
                count(case when a.red_packet_id>0 then a.id end) as use_red_packet_times,
                count(distinct case when a.red_packet_id>0 then uid end) as use_red_packet_users,
                
                sum(a.pm_buy/100.00) as cost_money,
                
                count(distinct a.recharge_id) as prize_recharge_times,
                count(distinct case when a.recharge_id>0 then a.uid end) as prize_recharge_users,
                
                sum(a.recovery_num) as recovery_num,
                count(case when a.recovery_num>0 then a.id end) as recovery_times,
                count(distinct case when a.recovery_num>0 then a.uid end) as recovery_users,
                
                sum(a.auto_recovery_num) as auto_recovery_num,
                count(case when a.auto_recovery_num>0 then a.id end) as auto_recovery_times,
                count(distinct case when a.auto_recovery_num>0 then a.uid end) as auto_recovery_users,
            
                sum(a.manual_recovery_num) as manual_recovery_num,
                count(case when a.manual_recovery_num>0 then a.id end) as manual_recovery_times,
                count(distinct case when a.manual_recovery_num>0 then a.uid end) as manual_recovery_users
            from 
            (
                select m_type,prize_type,is_free,id,pm,uid,pm_red_packet,red_packet_id,pm_buy,recharge_id,recovery_num,auto_recovery_num,manual_recovery_num,
            ...
            (	-- 存在多抽对应同一个商户订单号
                ...
                where to_char(addtime,'yyyy-mm-dd') between '{0}' and to_char(date '{1}' + integer '1','yyyy-mm-dd')
            ) p 
            ...
            group by grouping sets((1),(1,2),(1,3),(1,4),(1,5),(1,6),(1,3,4))
        ) t 
        ...
        # 目标表 数据写入
        columns = 24
        insert_sql = "insert into wow_dwm_war_measures_by_prizeday(part_day ,pay_method ,source_name ,model_name ,sxtype_name ,is_free ,prize_money ,prize_times ,prize_users ,use_red_packet_money ,use_red_packet_times ,use_red_packet_users ,cost_money ,prize_recharge_times ,prize_recharge_users ,recovery_num ,recovery_times ,recovery_users ,auto_recovery_num ,auto_recovery_times ,auto_recovery_users ,manual_recovery_num ,manual_recovery_times ,manual_recovery_users) values ({0});".format(','.join(['%s'] * columns))
...
if __name__ == '__main__':
    ...
    for i in lst:
        ...
        wow_dwm_war_measures_by_prizeday.run(last_8day,last_day)
        end_time = time.time()
