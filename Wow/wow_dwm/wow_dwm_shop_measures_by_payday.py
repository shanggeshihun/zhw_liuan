...
# @Author    : An
# @File      : wow_dwm_shop_measures_by_payday.py
# @Software  : wow_dwm_shop_measures_by_payday 涉及 滞后更新的数据指标  涉及退款 已核对
...
class WowDwmShopMeasuresByPayday:
    def __init__(self):
    ...
    def run(self, start_day, end_day):
        ...
        # 清理目标表数据
        mysql_sql = "delete from wow_dwm_shop_measures_by_payday where part_day between '{0}' and '{1}'".format(start_day, end_day)
        operate_mysql.delete_data(mysql_sql)

        # 原始表 数据处理
        holo_sql = """
        select part_day ,coalesce(pay_method,'-') as pay_method,coalesce(source_name,'-') as source_name ,coalesce(sxtype_name,'-') as sxtype_name ,
        pay_money ,pay_wabao ,pay_give_wabao ,orders ,users ,pay_money_users ,pay_wabao_users ,pay_give_wabao_users,
        rf_pay_money ,rf_pay_wabao ,rf_pay_give_wabao ,rf_orders ,rf_users ,rf_pay_money_users ,rf_pay_wabao_users ,rf_pay_give_wabao_users ,
        cost ,buy_num ,rf_cost ,rf_buy_num ,no_rf_users
        from 
        (
            select 
                ...
                case 
                    when a.shop_type = 1 then '虚拟'
                    when a.shop_type = 2 then '实物' 
                    when a.shop_type = 3 then '红包' 
                    else '其他' 
                end as sxtype_name,
                sum(a.pm/100.00) as pay_money,
                sum(a.pm_wabao) as pay_wabao,
                sum(a.pm_give_wabao) as pay_give_wabao,
                count(a.uid) as orders,
                count(distinct a.uid) as users,
                count(distinct case when a.pm>0 then a.uid end) as pay_money_users,
                count(distinct case when a.pm_wabao>0 then a.uid end) as pay_wabao_users,
                count(distinct case when a.pm_give_wabao>0 then a.uid end) as pay_give_wabao_users,
                sum(case when a.status = 6 then a.pm/100.00 end) as rf_pay_money,
                sum(case when a.status = 6 then a.pm_wabao end) as rf_pay_wabao,
                sum(case when a.status = 6 then a.pm_give_wabao end) as rf_pay_give_wabao,
                count(case when a.status = 6 then a.uid end) as rf_orders,
                count(distinct case when a.status = 6 then a.uid end) as rf_users,
                count(distinct case when a.status = 6 and a.pm>0 then a.uid end) as rf_pay_money_users,
                count(distinct case when a.status = 6 and a.pm_wabao>0 then a.uid end) as rf_pay_wabao_users,
                count(distinct case when a.status = 6 and a.pm_give_wabao>0 then a.uid end) as rf_pay_give_wabao_users,
                sum(a.pm_buy*a.buy_num/100.00) as cost,
                sum(a.buy_num) as buy_num,
                sum(case when a.status = 6 then a.pm_buy*a.buy_num/100.00 end) as rf_cost,
                sum(case when a.status = 6 then a.buy_num end) as rf_buy_num,
                count(distinct case when a.status <> 6 then a.uid end) as no_rf_users
            from 
            (
                select shop_type,pm,pm_wabao,pm_give_wabao,uid,pm_buy,buy_num,pay_type,
        ...
        # 目标表 数据写入
        columns = 25
        insert_sql = "insert into wow_dwm_shop_measures_by_payday(part_day ,pay_method ,source_name ,sxtype_name ,pay_money ,pay_wabao ,pay_give_wabao ,orders ,users ,pay_money_users ,pay_wabao_users ,pay_give_wabao_users ,rf_pay_money ,rf_pay_wabao ,rf_pay_give_wabao ,rf_orders ,rf_users ,rf_pay_money_users ,rf_pay_wabao_users ,rf_pay_give_wabao_users ,cost ,buy_num ,rf_cost ,rf_buy_num ,no_rf_users) values ({0});".format(
            ','.join(['%s'] * columns))
...
if __name__ == '__main__':
    ...
    for i in lst:
        ...
        wow_dwm_shop_measures_by_payday.run(last_8day, last_day)
        end_time = time.time()
