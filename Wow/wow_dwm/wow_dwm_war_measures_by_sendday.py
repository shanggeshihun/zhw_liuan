...
# @Author    : An
# @File      : wow_dwm_war_measures_by_sendday.py
# @Software  : wow_dwm_war_measures_by_sendday 不涉及 滞后更新的数据指标，注意抽奖超过90日才发货 已核对
...
class WowDwmWarMeasuresBySendday:
    def __init__(self):
    ...
    def run(self, start_day, end_day):
        ...
        # 清理目标表数据
        mysql_sql = "delete from wow_dwm_war_measures_by_sendday where part_day between '{0}' and '{1}'".format(start_day, end_day)
        ...
        (
            select 
                ...
                case when p.id is null then '其他' else d1.item_name end as pay_method,
            ...
            from 
            (
                select m_type,prize_type,id,uid,recharge_id,war_id,
        ...
        # 目标表 数据写入
        columns = 15
        insert_sql = "insert into wow_dwm_war_measures_by_sendday(part_day ,pay_method ,source_name ,model_name ,sxtype_name ,is_free ,prize_money ,prize_times ,prize_users ,use_red_packet_money ,use_red_packet_times ,use_red_packet_users ,cost_money ,prize_recharge_times ,prize_recharge_users) values ({0});".format(','.join(['%s'] * columns))
...
if __name__ == '__main__':
    ...
    for i in lst:
        ...
        wow_dwm_war_measures_by_sendday.run(last_day, last_day)
        end_time = time.time()
