...
# @Author    : An
# @File      : wow_dwm_recharge_measures_by_addday.py
# @Software  : wow_dwm_war_measures_by_applysendday 涉及 滞后更新的数据指标 退款，注意抽奖超过90日才发货
...
class WowDwmRechargeMeasuresByAddday:
    def __init__(self):
    ...
    def run(self, start_day, end_day):
        ...
        # 清理目标表数据
        mysql_sql = "delete from wow_dwm_recharge_measures_by_addday where part_day between '{0}' and '{1}'".format(start_day, end_day)
        ...
        # 目标表 数据写入
        columns = 12
        insert_sql = "insert into wow_dwm_recharge_measures_by_addday(part_day ,pay_method ,source_name ,recharge_status ,big_business ,business ,recharge_money ,recharge_times ,recharge_users ,refund_money ,refund_times ,refund_users) values ({0});".format(','.join(['%s'] * columns))
...
if __name__ == '__main__':
    ...
    for i in lst:
        ...
        wow_dwm_recharge_measures_by_addday.run(last_8day, last_day)
        end_time = time.time()
