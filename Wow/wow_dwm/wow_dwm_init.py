...
# @Author    : An
# @File      : wow_dwm_init.py
# @Software  : wow_dwm_init 不涉及 滞后更新的数据指标;2022/12/26 添加 wow_dwm_init_appid 渠道包初始化表
...
class WowDwmInit:
    ...
    def run(self, start_day, end_day):
        ...
        # 清理目标表数据（日期初始化）
        mysql_sql = "delete from wow_dwm_init_days where part_day between '{0}' and '{1}'".format(start_day, end_day)
        operate_mysql.delete_data(mysql_sql)

        # 目标表 数据写入
        datestr_list = [m.strftime('%Y-%m-%d') for m in pd.date_range(start=start_day, end=end_day).tolist()]

        holo_data_list = [[d] for d in datestr_list]
        columns = 1
        insert_sql = "insert into wow_dwm_init_days(part_day) values ({0});".format(','.join(['%s'] * columns))
        ...
        # 清理目标表数据（渠道包初始化）
        mysql_sql = "delete from wow_dwm_init_appid where true ".format(start_day, end_day)
        operate_mysql.delete_data(mysql_sql)

        # 原始表 数据处理
        holo_sql = """
            select 
                cast(item_value as integer) as app_id,
                item_name as source_name
            from wow.wow_dict_item 
            where dict_id  = 3 
            and item_name not like '%预留%'
        ...
        # 目标表 数据写入
        columns = 2
        insert_sql = "insert into wow_dwm_init_appid(app_id,source_name) values ({0});".format(
            ','.join(['%s'] * columns))
        operate_mysql.insert_data(insert_sql, holo_data_list)
...
if __name__ == '__main__':
    ...
    for i in lst:
        ...
        wow_dwm_init.run(last_day, last_day)
        end_time = time.time()
