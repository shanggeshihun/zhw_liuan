...
# @Author    : An
# @File      : wow_dws_cost_by_applysendday_isreg.py
# @Software  : wow_dws_cost_by_applysendday_isreg 不涉及 滞后更新的数据指标 ，需要注意关联抽奖距离实际发货超过90天的情况
...
class WowDwsCostByApplysenddayIsreg:
    def __init__(self):
    ...
    def run(self, start_day ,end_day):
        ...
        # 清理目标表数据
        mysql_sql = "delete from wow_dws_cost_by_applysendday_isreg where part_day between '{0}' and '{1}'".format(start_day ,end_day)
        ...
        (
            ...
            (
                ...
                select uid,to_char(success_time,'yyyy-mm-dd') as part_day,'shop' as big_business,pm_buy*buy_num as pm_buy,pay_type,app_id
        ...
        # 目标表 数据写入
        columns = 7
        insert_sql = "insert into wow_dws_cost_by_applysendday_isreg(part_day,is_reg,pay_method,source_name,big_business,cost_money,send_users ) values ({0});".format(','.join(['%s'] * columns))
...
if __name__ == '__main__':
    ...
    for i in range(15,16):
        ...
        wow_dws_cost_by_applysendday_isreg.run(last_day ,last_day)
        end_time = time.time()
