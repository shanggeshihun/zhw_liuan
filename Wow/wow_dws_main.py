...
for i in lst:
    ...
    j = 1
    # wow_dws_rechage_after_reg_byaddday_gapdays 不涉及 滞后更新的数据指标  已核对
    ts = TaskSchedual()
    wow_dws_rechage_after_reg_byaddday_gapdays = WowDwsRechargeAfterRegByAdddayGapdays()
    class_instance, class_method, start_day ,end_day = 'wow_dws_rechage_after_reg_byaddday_gapdays', 'run', last_day ,last_day
    ts.run_task(class_instance, class_method, start_day ,end_day)
    time.sleep(1)
    print('\t',j, '\t wow_dws_rechage_after_reg_byaddday_gapdays')

    j += 1
    # wow_dws_login_after_reg_byloginday_gapdays 涉及 滞后更新的数据指标 ，注册第N日指标，N最多等于15 已核对
    ts = TaskSchedual()
    wow_dws_login_after_reg_byloginday_gapdays = WowDwsLoginAfterRegByALogindayGapdays()
    class_instance, class_method, start_day ,end_day = 'wow_dws_login_after_reg_byloginday_gapdays', 'run', last_15day ,last_day
    ts.run_task(class_instance, class_method, start_day ,end_day)
    time.sleep(1)
    print('\t',j, '\t wow_dws_login_after_reg_byloginday_gapdays')

    j += 1
    # wow_dws_cost_by_sendday_isreg 不涉及 滞后更新的数据指标 ，需要注意关联抽奖距离实际发货超过90天的情况 已核对
    ts = TaskSchedual()
    wow_dws_cost_by_sendday_isreg = WowDwsCostBySenddayIsreg()
    class_instance, class_method, start_day ,end_day = 'wow_dws_cost_by_sendday_isreg', 'run', last_day ,last_day
    ts.run_task(class_instance, class_method, start_day ,end_day)
    time.sleep(1)
    print('\t',j, '\t wow_dws_cost_by_sendday_isreg')

    j += 1
    # wow_dws_cost_by_applysendday_isreg 不涉及 滞后更新的数据指标 ，需要注意关联抽奖距离实际发货超过90天的情况 已核对
    ts = TaskSchedual()
    wow_dws_cost_by_applysendday_isreg = WowDwsCostByApplysenddayIsreg()
    class_instance, class_method, start_day ,end_day = 'wow_dws_cost_by_applysendday_isreg', 'run', last_day ,last_day
    ts.run_task(class_instance, class_method, start_day ,end_day)
    time.sleep(1)
    print('\t',j, '\t wow_dws_cost_by_applysendday_isreg')

    j += 1
    # wow_dws_rechage_combination_by_payday 涉及 滞后更新的数据指标（回收） 已核对
    ts = TaskSchedual()
    wow_dws_rechage_combination_by_payday = WowDwsRechargeCominationByPayday()
    class_instance, class_method, start_day ,end_day = 'wow_dws_rechage_combination_by_payday', 'run', last_8day ,last_day
    ts.run_task(class_instance, class_method, start_day ,end_day)
    print('\t',j, '\t wow_dws_rechage_combination_by_payday')

    j += 1
    # wow_dws_advertising_by_day_adid 涉及 滞后更新的数据指标 ，该主题涉及注册7日内的充值等数据 已核对
    ts = TaskSchedual()
    wow_dws_advertising_by_day_adid = WowDwsAdvertisingByDayAdid()
    class_instance, class_method, start_day ,end_day = 'wow_dws_advertising_by_day_adid', 'run', last_8day ,last_day
    ts.run_task(class_instance, class_method, start_day ,end_day)
    print('\t',j, '\t wow_dws_advertising_by_day_adid')

    j += 1
    # wow_dws_measures_by_regdate_gapdays 涉及 滞后更新的数据指标 ，该主题涉及注册第N日内的充值、采购成本、退款等数据，N最多15 已核对
    ts = TaskSchedual()
    wow_dws_measures_by_regdate_gapdays = WowDwsMeasuresByRegdateGapdays()
    class_instance, class_method, start_day ,end_day = 'wow_dws_measures_by_regdate_gapdays', 'run', last_15day,end_day
    ts.run_task(class_instance, class_method, start_day ,end_day)
