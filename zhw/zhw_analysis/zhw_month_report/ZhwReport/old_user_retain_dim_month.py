...
# @Time      : 2022/6/7  12:41
# @Author    : An
# @File      : old_user_retain_dim_month.py
# @Software  : PyCharm
...
class ZhwAnalysis:
    ...
    def old_user_retain_dim_month(self, part_month):
        """
        ...
        df_result.iloc[:,1:]
        df_result.to_excel(r"./zhw_rpt_data/old_user_retain_dim_month_{0}.xlsx".format(part_month))
...
if __name__ == '__main__':
    ...
    for part_month in part_month_list:
        ...
        zhw_analysis.old_user_retain_dim_month(part_month)
        end_time = time.time()
        print('old_user_retain_dim_month，运行耗时：%s s' % (end_time - start_time))
