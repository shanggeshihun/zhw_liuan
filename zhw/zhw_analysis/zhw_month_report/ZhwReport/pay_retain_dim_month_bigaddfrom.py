...
# @Time      : 2022/6/13  11:31
# @Author    : An
# @File      : pay_retain_dim_month_bigaddfrom.py
# @Software  : PyCharm


# _*_coding:utf-8 _*_
# @Time　　 : 2022/6/12 23:16
# @Author　 : liuan
# @File　 　: certusers_regusers_payusers_dim_month_apptype.py
# @Theme : PyCharm


# _*_coding:utf-8 _*_
# @Time　　 : 2022/6/12 23:02
# @Author　 : liuan
# @File　 　: certificate_users_dim_month_bigaddfrom.py
# @Theme : PyCharm
...
# _*_coding:utf-8 _*_
# @Time　　 : 2022/6/11 17:11
# @Author　 : liuan
# @File　 　: topgame_gmv_zcgmv_cdgmv_dim_month_game.py
# @Theme : PyCharm
...
class ZhwAnalysis:
    ...
    def pay_retain_dim_month_bigaddfrom(self, part_month):
        """
        ...
        presto_sql = """
            select 
                a.part_month as reg_month,
                a.lx,
                count(distinct a.jkx_userid) as reg_users,
                count(distinct b.userid) as next_month_pay_users
            from 
            (
                select 
                part_month,
                ...
                end as lx,jkx_userid,jkx_timer 
                from zhwdb.zhw_user 
            ...
            left join 
            (
                select part_month,userid,add_time 
                from zhwdb.zhw_dingdan
                where part_month='{0}'
                group by 1,2,3
            ) b 
            on a.jkx_userid=b.userid 
            and a.part_month = to_char(date(b.part_month || '-01') - interval '1' month,'yyyy-mm')
            group by 1,2
        """.format(part_month)
        ...
        df_result.to_excel(r"./zhw_rpt_data/pay_retain_dim_month_bigaddfrom_{0}.xlsx".format(part_month))
...
if __name__ == '__main__':
    ...
    for part_month in part_month_list:
        ...
        zhw_analysis.pay_retain_dim_month_bigaddfrom(part_month)
        end_time = time.time()
        print('pay_retain_dim_month_bigaddfrom，运行耗时：%s s' % (end_time - start_time))
