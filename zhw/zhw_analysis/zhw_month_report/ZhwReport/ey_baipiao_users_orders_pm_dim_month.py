...
# @Author　 : liuan
# @File　 　: ey_baipiao_users_orders_pm_dim_month.py
# @Theme : PyCharm
...
class ZhwAnalysis:
    ...
    def ey_baipiao_users_orders_pm_dim_month(self, part_month):
        """
        ...
        presto_sql = """
            select a.part_month,count(a.userid) as users,sum(a.cd) cd_orders,sum(a.cdm) as cd_pm
            from  
            (
                select 
                    part_month,
                    userid,
                    count(id) as xd,
                    count(case when zt !=2 then id end) as cd,
                    sum(case when zt !=2 then pm end) as cdm
                from zhwdb.zhw_dingdan 
                where part_month='{0}'
                group by 1,2
                having count(id)>=8 
                and count(case when zt !=2 then id end)*1.000/count(id)>=0.5
            ) a 
            group by 1
        """.format(part_month)
        ...
        df_result.to_excel(r"./zhw_rpt_data/ey_baipiao_users_orders_pm_dim_month_{0}.xlsx".format(part_month))
...
if __name__ == '__main__':
    ...
    for part_month in part_month_list:
        ...
        zhw_analysis.ey_baipiao_users_orders_pm_dim_month(part_month)
        end_time = time.time()
        print('ey_baipiao_users_orders_pm_dim_month，运行耗时：%s s' % (end_time - start_time))
