...
# @Theme : APP终端认证用疏忽，注册用户数，付支付且费用户数
...
class ZhwAnalysis:
    ...
    def certusers_regusers_payusers_dim_month_apptype(self, part_month):
        """
        ...
        presto_sql = """
            select 
                a.part_month,a.applx,
                count(case when a.jkx_rz='已认证' then a.jkx_userid end) as certificate_users,
            ...
            (
                ...
                from zhwdb.zhw_user 
                where part_month='{0}' 
                and jkx_lx in(2,3,14,15) -- 注册来源类型
            )a 
            left join 
            (
        ...
        df_result.to_excel(r"./zhw_rpt_data/certusers_regusers_payusers_dim_month_apptype_{0}.xlsx".format(part_month))
...
if __name__ == '__main__':
    ...
    for part_month in part_month_list:
        ...
        zhw_analysis.certusers_regusers_payusers_dim_month_apptype(part_month)
        end_time = time.time()
        print('certusers_regusers_payusers_dim_month_apptype，运行耗时：%s s' % (end_time - start_time))
