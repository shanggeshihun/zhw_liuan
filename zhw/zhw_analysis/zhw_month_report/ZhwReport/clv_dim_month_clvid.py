...
class ZhwAnalysis:
    ...
    def clv_dim_month_clvid(self, save_date_list):
        """
        :param part_day: 月份参数
        :return: 月份维度
        """
        ...
        df_result.iloc[:,2:]
        df_result.to_excel(r"./zhw_rpt_data/clv_dim_month_clvid_{0}_to{1}.xlsx".format(min(save_date_list), max(save_date_list)))
...
if __name__ == '__main__':
    ...
    save_date_list = ['2022-12-05']
    zhw_analysis = ZhwAnalysis()
    zhw_analysis.clv_dim_month_clvid(save_date_list)
    end_time = time.time()
    print('clv_dim_month_clvid，运行耗时：%s s' % (end_time - start_time))
