...
class ZhwAnalysis:
    ...
    def withdraw_orders_dim_month_tslx(self, part_month):
        """
        ...
        presto_sql = """
            select a.part_month,b.lx,count(b.did) as withdraw_orders
            from 
            (
        ...
        df_result.to_excel(r"./zhw_rpt_data/withdraw_orders_dim_month_tslx_{0}.xlsx".format(part_month))
...
if __name__ == '__main__':
    ...
    for part_month in part_month_list:
        ...
        zhw_analysis.withdraw_orders_dim_month_tslx(part_month)
        end_time = time.time()
        print('withdraw_orders_dim_month_tslx，运行耗时：%s s' % (end_time - start_time))
