...
class ZhwAnalysis:
    ...
    def gmv_zcgmv_cdgmv_dim_day_game(self, start_day, end_day):
        """
        ...
        presto_sql = """
            ...
            from 
            (
                select userid,id,pm,pmoney,part_day,gameid,zt
            ...
            (
                ...
                where format_datetime(add_time,'yyyy-MM-dd')  between '{0}' and '{1}'
            )d  
            on a.id=d.dingdan_id
            left join zhwdb.zhw_game_info e 
            on a.gameid = e.id
            group by 1,2,3
        ...
        df_result.to_excel(r"./daily_export_data/gmv_zcgmv_cdgmv_dim_day_game_{0}_to_{1}.xlsx".format(start_day, end_day))
...
if __name__ == '__main__':
    ...
    start_day, end_day = '2022-08-29', '2022-09-04'
    start_time = time.time()
    zhw_analysis = ZhwAnalysis()
    zhw_analysis.gmv_zcgmv_cdgmv_dim_day_game(start_day, end_day)
    end_time = time.time()
    print('gmv_zcgmv_cdgmv_dim_day_game，运行耗时：%s s' % (end_time - start_time))

    # start_day, end_day = '2022-06-13', '2022-06-19'
    # start_time = time.time()
    # zhw_analysis = ZhwAnalysis()
    # zhw_analysis.gmv_zcgmv_cdgmv_dim_day_game(start_day, end_day)
    # end_time = time.time()
    # print('gmv_zcgmv_cdgmv_dim_day_game，运行耗时：%s s' % (end_time - start_time))
    #
    # start_day, end_day = '2022-06-20', '2022-06-26'
    # start_time = time.time()
    # zhw_analysis = ZhwAnalysis()
    # zhw_analysis.gmv_zcgmv_cdgmv_dim_day_game(start_day, end_day)
    # end_time = time.time()
