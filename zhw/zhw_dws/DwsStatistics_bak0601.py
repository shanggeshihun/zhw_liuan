...
class Dws:
    ...
    def zhw_dws_msg_recall_statistics(self, part_day):
        ...
        # 清空当日数据  维度  日期+抽奖用户
        mysql_sql = "delete from zhw_dws_msg_recall_statistics where part_day between '{0}' and date_add(str_to_date('{0}','%Y-%m-%d'),interval 7 day)".format(part_day)
        ...
        sql_1 = '''
            ...
            (
                ...
                where part_day between '{0}' and cast(date_add('day',7,cast('{0}' as date)) as varchar)
        ...
        sql_2 = '''
            ...
            (
                ...
                from zhwdb.zhw_user_login_log_extend
                where part_day between '{0}' and cast(date_add('day',17,cast('{0}' as date)) as varchar)
                -- group by 1,2,3
            ) c ---红包有效期[1,10]内的活跃情况
        ...
        sql_6 = '''
            ...
            (
                ...
                (   -- 红包有效期[1,10]
                    ...
                    from zhwdb.zhw_hongbao_order 
                    where part_day between '{0}' and cast(date_add('day',15,cast('{0}' as date)) as varchar) 
                ) t2 
                join 
                (
        ...
        sql_7 = '''
            ...
            (
                select part_day,userid 
                from zhwdb.zhw_user_login_log_extend  
                where part_day between '{0}' and cast(date_add('day',18,cast('{0}' as date)) as varchar) 
                -- group by 1,2
            ) c  
            -- 7日活跃留存
        ...
        sql_9 = '''
            ...
            (
                ...
                from zhwdb.zhw_dingdan 
                where part_day between '{0}' and cast(date_add('day',30,cast('{0}' as date)) as varchar) 
            ) d 
            -- 7日内付费
            ...
            (
                select date(part_day) as dt3,id,userid,pm 
                from zhwdb.zhw_dingdan 
                where part_day between '{0}' and cast(date_add('day',35,cast('{0}' as date)) as varchar) 
            ) dd 
    ...
    def zhw_dws_push_recall_statistics(self, part_day):
        ...
        # 清空近8日数据
        mysql_sql = "delete from zhw_dws_push_recall_statistics where part_day between '{0}' and date_add(str_to_date('{0}','%Y-%m-%d'),interval 7 day)".format(
            part_day)
        operate_mysql.update_data(mysql_sql)
...
if __name__ == '__main__':
    ...
    # 短信召回效果——数据（每日 以7日为单位重跑）
    lst = lst = [8, 16, 24, 32, 40,48]
    lst.reverse()
    for i in lst:
        start_time = time.time()
    ...
    # PUSH召回效果——数据（每日 以7日为单位重跑）

    lst = [8, 16, 24, 32, 40,48]
    lst.reverse()
    for i in lst:
        start_time = time.time()
