...
# @Time      : 2022/8/15  13:00
# @Author    : An
# @File      : DwstStatistics.py
# @Software  : zhw_dws_msg_recall_statistics,zhw_dws_red_packet_type_id_msg,zhw_dws_push_recall_statistics。\
# 2022/8/5 将 【3日内下单】【3日红包下单】分开执行，【3日红包下单】 红包先关联红包下单再关联下单表，红包订单表 添加type/issue条件;添加使用红包复购；将sql2 和 SQL6 拆开，将 select * 指定字段，强关联的时候left 改join
# 2023/1/17 添加活动的红包类型以及红包ID，type 和 id 仍写入 red_packet_trigger_way ，活动红包效果数据写入 zhw_dws_activity_recall_statistics
...
class Dws:
    ...
    def zhw_dws_msg_recall_statistics(self, part_day):
        ...
        msg_config_df['packet_type_id'] = msg_config_df.packet_type.astype(str) + '-' + msg_config_df.packet_id.astype(str)

        packet_type_id_tuple = tuple(set(msg_config_df.packet_type_id))
        ...
        sql_1 = '''
            ...
            (
                ...
                and cast(type as varchar) || '-' || cast(issue as varchar) in {1}
            ) a
            left join 
            (
            ...
            group by 1,2,3
        '''.format(part_day, packet_type_id_tuple)
        ...
        presto_data_list_2_1 = operate_presto.query_data(sql_2_1)
        if not presto_data_list_2_1:
            df_2_1 = pd.DataFrame(
                {'part_day': ['2099-01-01'], 'type': [0], 'issue': [0], 'curr_user': [0]}
            )
        else:
            result_columns_2_1 = operate_presto.query_data_index()
            df_2_1 = pd.DataFrame(presto_data_list_2_1)
            df_2_1.columns = result_columns_2_1
        time.sleep(1)
        end_time = time.time()
        print('sql_2_1', end_time - start_time)
        ...
        presto_data_list_2_2 = operate_presto.query_data(sql_2_2)
        if not presto_data_list_2_2:
            df_2_2 = pd.DataFrame(
                {'part_day': ['2099-01-01'], 'type': [0], 'issue': [0], 'youxiao_user': [0]}
            )
        else:
            result_columns_2_2 = operate_presto.query_data_index()
            df_2_2 = pd.DataFrame(presto_data_list_2_2)
            df_2_2.columns = result_columns_2_2
        time.sleep(1)
        end_time = time.time()
        print('sql_2_2', end_time - start_time)
        ...
        presto_data_list_6_1 = operate_presto.query_data(sql_6_1)

        if not presto_data_list_6_1:
            df_6_1 = pd.DataFrame(
                {'part_day': ['2099-01-01'], 'type': [0], 'issue': [0], 'threeday_fufei_user': [0],
                 'threeday_fufei_num': [0], 'threeday_fufei_money': [0]
                 }
            )
        else:
            result_columns_6_1 = operate_presto.query_data_index()
            df_6_1 = pd.DataFrame(presto_data_list_6_1)
            df_6_1.columns = result_columns_6_1
        time.sleep(1)
        end_time = time.time()
        print('sql_6_1', end_time - start_time)
        ...
        presto_data_list_6_2 = operate_presto.query_data(sql_6_2)

        if not presto_data_list_6_2:
            df_6_2 = pd.DataFrame(
                {'part_day': ['2099-01-01'], 'type': [0], 'issue': [0], 'threeday_hb_fufei_user': [0],
                 'threeday_hb_fufei_num': [0], 'threeday_hb_fufei_money': [0]
                 }
            )
        else:
            result_columns_6_2 = operate_presto.query_data_index()
            df_6_2 = pd.DataFrame(presto_data_list_6_2)
            df_6_2.columns = result_columns_6_2
        time.sleep(1)
        end_time = time.time()
        print('sql_6_2', end_time - start_time)
        ...
        sql_22 = '''
            ...
            from 
            (
                select part_day,type,issue,id,recetime,outtime
                from zhwdb.zhw_hongbao 
                where part_day between '{0}' and cast(date_add('day',3,cast('{0}' as date)) as varchar)
                and cast(type as varchar) || '-' || cast(issue as varchar) in {1}
            ) a 
        ...
        result = pd.merge(df_1, df_2_1, on=['part_day', 'type', 'issue'], how='left')
        result = pd.merge(result, df_2_2, on=['part_day', 'type', 'issue'], how='left')
        result = pd.merge(result, df_3, on=['part_day', 'type', 'issue'], how='left')
        result = pd.merge(result, df_5, on=['part_day', 'type', 'issue'], how='left')
        result = pd.merge(result, df_6_1, on=['part_day', 'type', 'issue'], how='left')
        result = pd.merge(result, df_6_2, on=['part_day', 'type', 'issue'], how='left')
    ...
    def zhw_dws_activity_recall_statistics(self, part_day):
        """
        :param part_day: 日期参数
        :return: PUSH 召回 统计数据写到MySQL
        """
        ...
        # 清空近8日数据
        mysql_sql = "delete from zhw_dws_activity_recall_statistics where part_day between '{0}' and date_add(str_to_date('{0}','%Y-%m-%d'),interval 3 day)".format(
            part_day)
        operate_mysql.update_data(mysql_sql)
        operate_mysql.close_conn()
        ...
        if presto_data_list:
            ...
            for i in range(r):
                tmp_list = presto_data_list[i * step:(i + 1) * step]
                batch_sql = ','.join([str(tuple(a)) for a in tmp_list])
                insert_sql = "insert into zhw_dws_activity_recall_statistics(part_day ,type ,issue ,hb_rece_user ,hb_rece_num ,hb_rece_money ,hb_use_user ,hb_use_num ,hb_use_money ,curr_user ,youxiao_user ,curr_fufei_user ,curr_fufei_num ,curr_fufei_money ,curr_hb_did_user ,curr_hb_did_num ,curr_hb_did_money ,threeday_login_user ,threeday_fufei_user ,threeday_fufei_num ,threeday_fufei_money ,threeday_hb_fufei_user ,threeday_hb_fufei_num ,threeday_hb_fufei_money ,sevenday_login_user ,fifteen_login_user ,sevenday_fufei_user ,fifteen_fufei_user,youxiao_fufei_user ,youxiao_fufei_num ,youxiao_fufei_money ,youxiao_hb_fufei_user ,youxiao_hb_fufei_num ,youxiao_hb_fufei_money) values {0};".format(
                    batch_sql)
                operate_mysql2.insert_data(insert_sql)
        operate_mysql2.close_conn()
