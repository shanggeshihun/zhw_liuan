...
"""
20231103 1138 新商户初始化-拒付维度新增下钻维度website_type
"""
...
for idx, item in df_merch.iterrows():
    ...
    tmp_merch_sql = '''
        select
            a.merchant_no,a.card_type,a.channel,a.bin_country,a.mcc,a.website_type,
    ...
    df_merch_init['attr_concat'] = df_merch_init.apply(
        lambda row: "{}-{}-{}-{}-{}".format(row['card_type'], row['channel'], row['bin_country'], row['mcc'], row['website_type']), axis=1
    )
    ...
    tmp_old_merch_sql = '''
        ...
        (
            ...
            (
                ...
                and concat(card_type,'-',channel,'-',bin_country,'-',mcc, '-', website_type) in (
                    {3}
                )
                group by 1,2
            ) a 
...
df_pred_chargeback_rate.to_excel('./df_pred_chargeback_rate_dim2.xlsx')
...
df_pred_r_rate.to_excel('./df_pred_cb_r_rate_dim2.xlsx')
