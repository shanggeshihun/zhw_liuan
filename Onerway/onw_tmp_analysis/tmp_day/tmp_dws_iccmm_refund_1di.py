...
# @Author    : An
# @File      : tmp_dws_iccmm_refund_1di.py
# @Software  : 收单退款宽表-退款日期
...
class TmpDwsIccmmRefund1di:
    ...
    def run(self, start_day ,end_day):
        ...
        # 清理目标表数据
        delete_sql = "delete from tmp.tmp_dws_iccmm_refund_1di where refund_day between '{0}' and '{1}'".format(start_day, end_day)
        operate_doris.delete_data(delete_sql)

        query_sql = """
            select 
                'V3' as version,
                date_format(t.txn_date,'%Y-%m-%d') as refund_day,
                ...
                count(distinct t.origin_txn_id) as refund_orders,
                count(distinct t.id) as refund_times,
                sum(t.order_amount * r.mid_rate) refund_money    
        ...
        # 目标表 数据写入
        column_concat = 'version, refund_day, institution, card_type, channel, MID, merchant_no, refund_orders, refund_times, refund_money'
        insert_sql = "insert into tmp.tmp_dws_iccmm_refund_1di({0}) values ({1});".format(
            column_concat,
...
if __name__ == '__main__':
    ...
    for idx, start_end_tuple in enumerate(start_end_day_list):
        ...
        start_time = time.time()
        tmp_dws_iccmm_refund_1di = TmpDwsIccmmRefund1di()
        tmp_dws_iccmm_refund_1di.run(start_day, end_day)
        end_time = time.time()
