...
holiday_name,theme,dim_desc,value_desc,dim_text_1,value_int_1,value_int_2,
...
select
    ...
    '无门槛红包' as theme,
    'dim_text_1 as userid' as dim_desc,
    'value_int_1 as days, value_int_2 as receive_hbids' as value_desc,
    a.userid as dim_text_1,a.days as value_int_1,a.receive_hbids as value_int_2,
...
(
    ...
    max(receive_hbids) as max_receive_hbids
    from user_receive_hongbao_info
    group by 1
    having count(distinct part_day)>=5 or sum(receive_hbids)>=50 or max(receive_hbids)>=12
    -- 无门槛红包0705-0718 共计 14天，每日最高领取24个
) a
