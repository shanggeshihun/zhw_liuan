...
select
    ...
    '无门槛红包' as theme,
    'dim_text_1 as part_day,dim_text_2 as hour' as dim_desc,
    'value_int_1 as users, value_int_2 as lag1_users' as value_desc,
    c.part_day as dim_text_1,c.hour as dim_text_2,c.users as value_int_1,c.lag1_users as value_int_2,
    '{0}' as param_start_day,'{1}' as param_end_day,0 as record_deal_flag,current_timestamp as push_time
from
(
	select part_day,hour,users,lag(users,1)over(order by part_day asc,hour asc) as lag1_users
	from
	(
		select a.part_day,hour,count(distinct userid) as users
		from
		(
			select a.jkx_userid as userid,
				a.part_day,
				to_char(to_timestamp(a.recetime),'hh24') as hour
			from ods_zhw.zhw_hongbao a
			join no_limit_hongbao b
			on a.issue = b.issue and a.type = b.type
			where true
...
where lag1_users>10 and users*1.00/lag1_users>=4
