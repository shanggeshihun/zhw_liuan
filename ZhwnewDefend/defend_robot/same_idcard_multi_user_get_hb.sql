...
select
    ...
    '无门槛红包' as theme,
    'dim_text_1 as jkx_usercard,dim_text_2 as card_users,dim_text_3 as userid,dim_text_4 as sh_type' as dim_desc,
    'value_int_1 as days, value_int_2 as receive_hbids' as value_desc,
    jkx_usercard as dim_text_1,card_users as dim_text_2,userid as dim_text_3,sh_type as dim_text_4,days as value_int_1,receive_hbids as value_int_2,
    '{0}' as param_start_day,'{1}' as param_end_day,0 as record_deal_flag,current_timestamp as push_time
from
(
    -- 结果1：同身份证多用户领取情况
    select b.jkx_usercard,b.users as card_users,a.userid,coalesce(c.sh_type,'普通用户') as sh_type,a.days,a.receive_hbids
    from
    (
        select userid,jkx_usercard,count(distinct part_day) as days,sum(receive_hbids) as receive_hbids
        from user_receive_hongbao_info
        group by 1,2
    ) a
    join
    (
        select jkx_usercard,count(distinct a.userid) as users,sum(receive_hbids) receive_hbids
        from user_receive_hongbao_info a
        where jkx_usercard<>'' and jkx_usercard is not null
        group by 1
        having count(distinct a.userid)>1
    ) b
    on a.jkx_usercard = b.jkx_usercard
    left join huserid_identity c
    on a.jkx_usercard = c.userid
    order by 1,3,4
) a
